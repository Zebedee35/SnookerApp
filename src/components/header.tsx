...
const HeaderBottom: FC<THeaderBottomProps> = ({
  children,
  leftText,
  middleText,
  rightText,
  ...
  return (
    <Box style={{flexDirection: 'row', marginBottom: 5}} {...props}>
      <LabelHeader style={{fontSize: consts.fontSizes.listItem, marginLeft: 10}}>
        {leftText}
      </LabelHeader>
      {!middleText || middleText === '' ? (
        <Box style={{flex: 1}}>{children}</Box>
      ) : (
        <LabelHeader style={{fontSize: consts.fontSizes.listItem, flex: 1}}>
          {middleText}
        </LabelHeader>
      )}
      <LabelHeader style={{fontSize: consts.fontSizes.listItem, marginRight: 10}}>
        {rightText}
      </LabelHeader>
    </Box>
  );
