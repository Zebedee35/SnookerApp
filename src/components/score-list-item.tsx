...
const ScoreListItem: FC<TScoreListItemProp> = ({ item, bigSize = false, onPlayerSelected, onPVPSelected }) => {
  ...
  function checkMatchState(mstate: string) {
    switch (mstate) {
      ...
      case 'live': return <Live bigSize={bigSize} />
  ...
  function scoreSize() {
    return bigSize ? consts.fontSizes.scoreBig
      : consts.fontSizes.scoreSmall
  }
      ...
      <Box style={{ flexDirection: 'row', height: bigSize ? 80 : 50 }}>
        ...
        <Pressable style={{ width: bigSize ? 130 : 85, height: '100%' }} onPress={() => {
          ...
          <Box style={{ width: '100%', marginTop: 5, flexDirection: 'row' }}>
            <Label textType='bold' style={[styles.score, { fontSize: scoreSize(), textAlign: 'right' }]}>{item.players![0].score}</Label>
            <Label textType='bold' style={[styles.score, { fontSize: scoreSize(), textAlign: 'center' }]}>-</Label>
            <Label textType='bold' style={[styles.score, { fontSize: scoreSize(), textAlign: 'left' }]}>{item.players![1].score}</Label>
          </Box>
        ...
        <Pressable style={{ flex: 1, flexDirection: bigSize ? 'row-reverse' : 'row' }} onPress={() => {
          ...
          {!bigSize
            ? <Box style={{ flex: 1, margin: 3, justifyContent: 'center', alignItems: 'center' }}>
              <Label style={[styles.playerName, styles.playerNameRight]}>{item.players![1].name}</Label>
            </Box>
            : <></>}
          <Box style={{ width: bigSize ? 80 : 44, margin: 3 }}>
      ...
      {bigSize
        ? <Box style={{ height: 20, flexDirection: 'row' }}>
          <Box style={{ flex: 1, marginLeft: 3, justifyContent: 'center', alignItems: 'center' }} >
            <Label style={[styles.playerName, styles.playerNameLeft]}>{item.players![0].name}</Label>
          </Box>
